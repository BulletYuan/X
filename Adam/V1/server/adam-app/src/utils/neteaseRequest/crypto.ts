...
const aesEncrypt = (buffer, mode, key, iv1) => {
    const cipher = crypto.createCipheriv('aes-128-' + mode, key, iv1);
...
const rsaEncrypt = (buffer, key) => {
    ...
    const ck = 'constants';
    return crypto.publicEncrypt({ key, padding: crypto[ck].RSA_NO_PADDING }, buffer);
};
...
const weapi = (object) => {
    ...
    return {
        params: aesEncrypt(Buffer.from(aesEncrypt(Buffer.from(text),
            'cbc', presetKey, iv).toString('base64')), 'cbc', secretKey, iv).toString('base64'),
