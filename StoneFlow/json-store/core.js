...
const BulletJDB = (function () {
    ...
    if (this.state === 1) {
      ...
      for (let i = 0; i < _data[this.connection.tb].length; i++) {
        ...
        if (_wk.length > 0) {
          ...
          for (let x = 0; x < _wk.length; x++) {
            if (_item[_wk[x]] === query.where[_wk[x]]) {
              _tmp.push(_item);
  ...
  BulletJDB.prototype.insert = function (query) {
    if (this.state === 1) {
      ...
      const _self = this;
      let _data = utils.file.read(this.path);
      for (let i = 0; i < insert_query.datas.length; i++) {
        ((i) => {
          _data[_self.connection.tb].push(insert_query.datas[i]);

          // history report
          _data = utils.history.report(_data, 'create', { db: _self.connection.db, tb: _self.connection.tb }, _data[_self.connection.tb].length - 1, insert_query.datas[i]);
          utils.file.write(_self.path, _data);
        })(i);
      }
      return true;
  ...
  BulletJDB.prototype.update = function (query) {
    if (this.state === 1) {
      ...
      const _tmp = [], _wk = Object.keys(query.where), _d = Object.keys(query.data), _self = this;
      let _data = utils.file.read(this.path);
      // where filter
      for (let i = 0; i < _data[this.connection.tb].length; i++) {
        const _item = _data[_self.connection.tb][i];
  ...
  BulletJDB.prototype.delete = function (query) {
    if (this.state === 1) {
      ...
      const _wk = Object.keys(query.where), _d = query.keys, _self = this;
      let _data = utils.file.read(this.path);
      // where filter
      for (let i = 0; i < _data[this.connection.tb].length; i++) {
        const _item = _data[_self.connection.tb][i];
        ...
        if (_wk.length > 0) {
          for (let x = 0; x < _wk.length; x++) {
            if (_item[wk[x]] === query.where[wk[x]]) {
              _mark = true;
            } else {
              _mark = false;
              break;
